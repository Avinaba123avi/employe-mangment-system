    ...
    <div id="wrapper">
        ...
        @include('layouts.allnavigation')
            ...
            <div class="container-fluid">
                <h1 class="h3 mb-2 text-gray-800">Update Users Detais</h1>
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Update User</h6>
                </div>
                <div class="card-body">
                  <form action="{{ route('update.page',$users->regiuser_id)}}" class="user" method="POST">
                    ...
                    <div class="form-group row d-flex justify-content-center mt-3">
                        <div class="col-sm-4 mb-3 mb-sm-0">
                            <input type="text" class="form-control form-control-user   @error('first_name') is-invalid @enderror" id="first_name"
                                name="first_name" placeholder="First Name" value="{{ $users->first_name }}">
                                @error('first_name')
                    ...
                    <div class="form-group d-flex justify-content-center">
                      <div class="col-sm-8 mb-3 mb-sm-0">
                        <select class="form-control-user form-control form-select @error('usertype_id') is-invalid @enderror" id="usertype_id" name="usertype_id" placeholder="Select Role">
                            <option value="">Select Role</option>
                            <option value="1" {{ $users->usertype_id == 1 ? 'selected' : '' }}>Admin</option>
                            <option value="2" {{ $users->usertype_id == 2 ? 'selected' : '' }}>Manager</option>
                            <option value="3" {{ $users->usertype_id == 3 ? 'selected' : '' }}>Employee</option>
                        </select>
                        @error('usertype_id')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="text-center mb-3">
                      <button type="submit" href="/admincrud" class="btn btn-primary btn-user btn-lg">Update User</button>
                    </div> 
                </form>
                </div>
            </div>
